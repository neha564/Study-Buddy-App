...
// Fetch the booking ID and total amount from the GET request
$booking_id = isset($_GET['id']) ? $_GET['id'] : '';
$total_amount = isset($_GET['total_amount']) ? $_GET['total_amount'] : 0;

// Check if the booking ID and total amount are provided
if ($booking_id && $total_amount) {
    // If both are present, show the booking details
    $booking_message = "Booking ID: " . $booking_id . "<br>Total Amount: ₹" . number_format($total_amount, 2);
} else {
    // If any parameter is missing, show an error message
    $booking_message = "Error: Missing booking ID or total amount!";
}
?>
...
<div class="container">
    ...
    <!-- Payment Methods -->
    <?php if ($booking_id && $total_amount): ?>
...
<script>
    ...
    function processPayment(paymentMethod) {
        ...
        if (paymentMethod === 'credit-card') {
            ...
            if (cardNumber && expiryDate && cvv) {
                window.location.href = "payment_success.php?payment_method=credit-card&booking_id=" + bookingId + "&total_amount=" + totalAmount;
            } else {
            ...
            if (bankName) {
                window.location.href = "payment_success.php?payment_method=net-banking&booking_id=" + bookingId + "&total_amount=" + totalAmount;
            } else {
    ...
    function payWithUPI(upiMethod) {
        const bookingId = "<?php echo $booking_id; ?>";
        const totalAmount = "<?php echo $total_amount; ?>";
        window.location.href = "payment_success.php?payment_method=" + upiMethod + "&booking_id=" + bookingId + "&total_amount=" + totalAmount;
    }
</script>
